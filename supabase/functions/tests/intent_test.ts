...
const CASES: RuleCase[] = [
  ...
  // tournament_search — "협회장배"(장배 접미사). "대회" 단어가 없어도 대회로 인식.
  //   "서구" 는 region 별칭이 아니므로 슬롯 비어야 함 → 전 종목 검색 경로.
  {
    msg: '서구 협회장배 언제야?',
    intent: 'tournament_search',
    slots: {},
  },
  // tournament_search — "협회장기"(기 접미사) + 종목 슬롯
  {
    msg: '협회장기 풋살대회 일정',
    intent: 'tournament_search',
    slots: { sport: 'futsal' },
...
Deno.test(`classifyByRule: ${CASES.length}개 정형 질문이 기대 의도로 분류된다`, () => {
...
Deno.test('routing 가능 여부: rule 분류 tournament_search 만 LLM 우회 대상', () => {
  ...
  // tournament_search 룰 매칭만 LLM 없이 routing 된다.
  // (대회+상세 = tournament_detail 은 제외, 대회/대회명 접미사 키워드만 routable)
