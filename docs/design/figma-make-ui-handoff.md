...
| 높음 | [전국 테니스·풋살 구장 정보 화면 및 데이터 연동 기획 #14](https://github.com/kimabba/Match-up/issues/14) | 디자인 화면과 백엔드 데이터/API 범위를 분리해 합의한 뒤 구현 필요 |
...
- `#14`는 아직 앱 코드나 데이터베이스에 구현한 기능이 아닙니다.
- 구장 목록·상세 화면 디자인은 디자인 작업으로 먼저 진행할 수 있습니다.
- 구장 검색, 저장, 지도 좌표, 이미지 보관, 정보 검수는 백엔드 설계 확정 후 연결해야 합니다.
